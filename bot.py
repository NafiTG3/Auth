...
ADMIN_GROUP_ID      = int(os.environ.get("GROUP_ID", "0"))
...
TOTP_NAME_MAX_LEN      = 20
MAX_DAILY_LOGINS       = 7
MAX_WEEKLY_SIGNUPS     = 2
MAX_LIFETIME_VAULTS    = 5
MAX_TOTP_PER_MINUTE    = 20

# ── Bot-wide toggleable settings ───────────────────────────
_bot_settings: dict = {
    "maintenance": False,
    "signup_enabled": True,
    "login_enabled": True,
    ...
    if kdf_type != "argon2id" or not has_mk:
        try:
            ...
            with get_db() as c:
                ...
                sk = load_user_secure_key(vid, pw)
                if sk:
    ...
    with get_db() as c:
        ...
        if not sk_skipped:
            ...
            if u_row and u_row["mk_enc"] and secure_key:
                ...
                if secure_key:
                    vault_key = _get_vault_key(vid, new_pw)
                    ct, s, iv = encrypt(secure_key, vault_key, vid)
                    c.execute(
                        "UPDATE users SET sk_enc=?, sk_salt=?, sk_iv=? WHERE vault_id=?",
                        (ct, s, iv, vid),
                    )
        ...
        with get_db() as c:
            ...
            for row in rows:
                try:
                    ...
                    vault_key_new = _get_vault_key(vault, new_pw)
                    ct, s, iv = encrypt(secret, vault_key_new, vault)
                    c.execute("UPDATE totp_accounts SET secret_enc=?, salt=?, iv=? WHERE id=?",
                              (ct, s, iv, row["id"]))
                except Exception as e:
            ...
            c.execute(
                "UPDATE users SET password_hash=?, pw_salt=?, kdf_type=? WHERE vault_id=?",
                (hash_pw(new_pw, new_salt, "argon2id"), new_salt, "argon2id", vault),
            )
            sk = load_user_secure_key(vault, old_pw)
            if sk:
                vault_key_new = _get_vault_key(vault, new_pw)
                ct, s, iv = encrypt(sk, vault_key_new, vault)
...
async def change_pw_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if u and u["mk_enc"]:
        ...
        with get_db() as c:
            ...
            c.execute(
                "UPDATE users SET password_hash=?, pw_salt=?, kdf_type=? WHERE vault_id=?",
                (hash_pw(new_pw, ns, "argon2id"), ns, "argon2id", vault),
            )
            sk = load_user_secure_key(vault, old_pw)
            if sk:
                ct, s, iv = encrypt(sk, vault_key_new, vault)
