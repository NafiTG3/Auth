...
MAX_LOGIN_ATTEMPTS  = 5     # new: max failed logins per account before freeze
LOGIN_FREEZE_HOURS  = 18    # new: freeze duration after too many failed logins
TOTP_PER_PAGE       = 10    # new: TOTP entries per page in list view
NOTE_MAX_LEN        = 10    # new: max note characters
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.args:
        ...
        # After showing shared codes, stay in current state (no vault change)
...
def main():
    ...
    init_db()
    purge_expired_share_links()  # clean any leftover expired links on startup
    ...
    # ── Daily job: send backup reminders ──────────────────────
