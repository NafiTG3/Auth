...
GROUP_ID           = int(os.environ.get("GROUP_ID", "0"))  # admin group chat id
...
def init_db():
    with get_db() as c:
        ...
        c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('maintenance', '0')")
        c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('signup_enabled', '1')")
        c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('login_enabled', '1')")

        # Migrations
        for col, defval in [("tg_name", "''"), ("timezone", "'UTC'"), ("last_active", "0"), ("disabled", "0")]:
            try:
        ...
        for col, coltype, default in [("note","TEXT","''"), ("account_type","TEXT","'totp'"), ("hotp_counter","INTEGER","0")]:
            try:
...
async def delete_after(msg, delay=30):
...
def generate_code(account_type: str, secret: str, hotp_counter: int = 0):
    if account_type == "steam":
        ...
        # next code for steam (next 30s window)
        STEAM_CHARS = "23456789BCDFGHJKMNPQRTVWXY"
        c = clean_secret(secret)
        k = base64.b32decode(c + "=" * ((8 - len(c) % 8) % 8))
        ts = int(time.time())
...
def record_login_failure(vault_id: str) -> bool:
    now = int(time.time())
    with get_db() as c:
        row = c.execute("SELECT attempts, frozen_until FROM login_attempts WHERE vault_id=?", (vault_id,)).fetchone()
        if row and row["frozen_until"] > now: return True
        attempts = (row["attempts"] if row else 0) + 1
        frozen_until = now + LOGIN_LOCK_HOURS * 3600 if attempts >= MAX_LOGIN_ATTEMPTS else 0
        c.execute("INSERT OR REPLACE INTO login_attempts (vault_id, attempts, frozen_until) VALUES (?,?,?)",
                  (vault_id, attempts, frozen_until))
        c.commit()
        return frozen_until > now
...
def get_login_freeze_remaining(vault_id: str) -> tuple[int,int]:
    with get_db() as c:
...
def is_signup_enabled() -> bool:
    return get_setting("signup_enabled") == "1"

def is_login_enabled() -> bool:
    return get_setting("login_enabled") == "1"

def is_account_disabled(vault_id: str) -> bool:
    with get_db() as c:
        row = c.execute("SELECT disabled FROM users WHERE vault_id=?", (vault_id,)).fetchone()
        return row and row["disabled"] == 1

def update_user_last_active(telegram_id: int):
    with get_db() as c:
        c.execute("UPDATE users SET last_active=? WHERE telegram_id=?", (int(time.time()), telegram_id))
        c.commit()
...
def build_search_results_kb(results: list, page: int, total_pages: int) -> InlineKeyboardMarkup:
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"search_page_{page-1}"))
    nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="search_noop"))
    if page < total_pages-1:
        nav.append(InlineKeyboardButton("Next ➡️", callback_data=f"search_page_{page+1}"))
    kb = [nav] if nav else []
    kb.append([InlineKeyboardButton("🔍 New Search", callback_data="search_totp")])
...
def get_user_by_username(username: str):
    ...
    with get_db() as c:
        return c.execute("SELECT * FROM users WHERE tg_name LIKE ?", (f"%{username}%",)).fetchone()
...
async def check_backup_reminder(bot, vault_id: str, telegram_id: int):
    with get_db() as c:
        row = c.execute("SELECT last_sent, frequency, enabled FROM backup_reminders WHERE telegram_id=?", (telegram_id,)).fetchone()
        if not row or not row["enabled"]: return
        last = row["last_sent"] or 0
        now = int(time.time())
        interval = 7*24*3600 if row["frequency"] == "weekly" else 30*24*3600
        if now - last >= interval:
            try:
                ...
                with get_db() as c2:
                    c2.execute("UPDATE backup_reminders SET last_sent=? WHERE telegram_id=?", (now, telegram_id))
                    c2.commit()
            except Exception as e:
...
async def backup_reminder_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        row = c.execute("SELECT frequency, enabled FROM backup_reminders WHERE telegram_id=?", (uid,)).fetchone()
    freq = row["frequency"] if row else "weekly"
    enabled = bool(row["enabled"]) if row else True
    status = "🟢 Enabled" if enabled else "🔴 Disabled"
    freq_lbl = "📅 Weekly" if freq == "weekly" else "📆 Monthly"
    await q.edit_message_text(
        f"🔔 *Backup Reminder*\n\nStatus: {em(status)}\nFrequency: {em(freq_lbl)}\n\n_Regular backups protect your TOTP data._",
        parse_mode="MarkdownV2",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔕 Disable" if enabled else "🔔 Enable", callback_data="backup_rem_toggle")],
            [InlineKeyboardButton("📆 Switch to Monthly" if freq == "weekly" else "📅 Switch to Weekly", callback_data="backup_rem_freq")],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
        ]))
    return TOTP_MENU
...
async def backup_rem_toggle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        ...
        c.execute("INSERT OR REPLACE INTO backup_reminders (telegram_id, enabled) VALUES (?,?)", (uid, new_enabled))
        c.commit()
...
async def backup_rem_freq(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        ...
        c.execute("INSERT OR REPLACE INTO backup_reminders (telegram_id, frequency) VALUES (?,?)", (uid, new_frq))
        c.commit()
    return await backup_reminder_menu(update, ctx)
...
def is_admin_group(update: Update) -> bool:
    ...
    return chat_id == GROUP_ID and GROUP_ID != 0
...
async def maintenance_on(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    set_setting("maintenance", "1")
    await update.message.reply_text("🔧 Maintenance mode ENABLED. Users cannot interact with the bot.")
...
async def export_all_data(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    args = ctx.args
    if not args:
    ...
    enc_pw = " ".join(args)
    if len(enc_pw) < 4:
        await update.message.reply_text("Password too short (min 4 characters).")
        return
    with get_db() as c:
        users = c.execute("SELECT telegram_id, vault_id, tg_name, created_at, disabled, timezone FROM users").fetchall()
        totp_counts = {}
        for u in users:
            cnt = c.execute("SELECT COUNT(*) as n FROM totp_accounts WHERE vault_id=?", (u["vault_id"],)).fetchone()["n"]
            totp_counts[u["vault_id"]] = cnt
    export_data = []
    for u in users:
        export_data.append({
            ...
            "disabled": u["disabled"],
            "timezone": u["timezone"],
            "totp_count": totp_counts.get(u["vault_id"], 0)
        })
    plain = json.dumps({"export_time": int(time.time()), "users": export_data}, ensure_ascii=False).encode()
    payload = export_encrypt(plain, enc_pw)
    bio = BytesIO(payload)
    timestamp = datetime.datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    filename = f"bv_full_export_{timestamp}.bvault"
    await update.message.reply_document(document=bio, filename=filename,
        caption="🔒 *Full user database export*\nUse /import to restore.")
    await update.message.reply_text("✅ Export complete.")

async def import_all_data(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    ctx.user_data["admin_import_waiting"] = True
    await update.message.reply_text("📥 Send the *.bvault* export file.")

async def admin_import_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.user_data.get("admin_import_waiting"): return
    if not update.message.document:
        await update.message.reply_text("Please send the backup file.")
        return
    ...
    if len(payload) < 28:
        ...
        return
    ctx.user_data["admin_import_payload"] = payload
    await update.message.reply_text("Enter the encryption password used during export:")
    ctx.user_data["admin_import_waiting"] = False
    ctx.user_data["admin_import_pw_waiting"] = True

async def admin_import_pw(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.user_data.get("admin_import_pw_waiting"): return
    pw = update.message.text.strip()
    payload = ctx.user_data.pop("admin_import_payload", None)
    ctx.user_data.pop("admin_import_pw_waiting", None)
    if not payload:
        await update.message.reply_text("Session expired. Send /import again.")
        return
    try:
        plain = export_decrypt(payload, pw)
        data = json.loads(plain.decode())
        users_data = data.get("users", [])
    except Exception:
        await update.message.reply_text("❌ Decryption failed. Wrong password or corrupted file.")
        return
    # Import users (skip existing, update only name, timezone, disabled)
    with get_db() as c:
        for ud in users_data:
            vault_id = ud["vault_id"]
            existing = c.execute("SELECT 1 FROM users WHERE vault_id=?", (vault_id,)).fetchone()
            if existing:
                c.execute("UPDATE users SET tg_name=?, timezone=?, disabled=? WHERE vault_id=?",
                          (ud.get("name",""), ud.get("timezone","UTC"), ud.get("disabled",0), vault_id))
            else:
                # Cannot import because we don't have password hash. Skip.
                continue
        c.commit()
    await update.message.reply_text(f"✅ Import completed. {len(users_data)} user records processed (existing updated).")

async def user_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    if not ctx.args:
        await update.message.reply_text("Usage: /user <telegram_id|vault_id|@username>")
        return
    ...
    if identifier.startswith("@"):
        ...
        user = get_user_by_tid(int(identifier))
    else:
    ...
    # Get additional data
    with get_db() as c:
        totp_count = c.execute("SELECT COUNT(*) as n FROM totp_accounts WHERE vault_id=?", (vault_id,)).fetchone()["n"]
        login_fail_row = c.execute("SELECT attempts FROM login_attempts WHERE vault_id=?", (vault_id,)).fetchone()
        login_fail = login_fail_row["attempts"] if login_fail_row else 0
        reset_fail_row = c.execute("SELECT attempts FROM reset_attempts WHERE vault_id=?", (vault_id,)).fetchone()
        reset_fail = reset_fail_row["attempts"] if reset_fail_row else 0
        reminder = c.execute("SELECT frequency, enabled FROM backup_reminders WHERE telegram_id=?", (telegram_id,)).fetchone()
        rem_status = "off"
        if reminder:
            if reminder["enabled"]:
                rem_status = reminder["frequency"]  # weekly/monthly
            else:
                rem_status = "off"
        # Last active
        last_active = user["last_active"] or 0
        last_active_str = datetime.datetime.fromtimestamp(last_active).strftime("%Y/%m/%d-%H:%M:%S") if last_active else "Never"
    # Timezone for creation date
    tz = user["timezone"] or "UTC"
    created_str = fmt_time(user["created_at"], tz) if user["created_at"] else "Unknown"
    status = "Good" if not user["disabled"] else "Disabled"
    text = (
        f"👤 *User Info*\n\n"
        f"*Vault ID:* `{vault_id}`\n"
        f"*Telegram Username:* {em(identifier)}\n"
        f"*Name:* {em(user['tg_name'] or 'Unknown')}\n"
        f"*Total TOTP Added:* {totp_count} Account\n"
        f"*Vault Created Date:* {created_str}\n"
        f"*Last Online:* {last_active_str}\n"
        f"*Account Status:* {status}\n"
        f"*Reminder Status:* {rem_status}\n"
        f"*Failed Login:* {login_fail}\n"
        f"*Failed Reset:* {reset_fail}\n"
    )
    await update.message.reply_text(text, parse_mode="MarkdownV2")

async def signup_toggle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    args = ctx.args
    if not args or args[0] not in ("on","off"):
        await update.message.reply_text("Usage: /sign-up on   or   /sign-up off")
        return
    new_val = "1" if args[0] == "on" else "0"
    set_setting("signup_enabled", new_val)
    await update.message.reply_text(f"✅ Sign-up {'enabled' if new_val=='1' else 'disabled'}.")

async def login_toggle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    args = ctx.args
    if not args or args[0] not in ("on","off"):
        await update.message.reply_text("Usage: /login on   or   /login off")
        return
    new_val = "1" if args[0] == "on" else "0"
    set_setting("login_enabled", new_val)
    await update.message.reply_text(f"✅ Login {'enabled' if new_val=='1' else 'disabled'}.")

async def account_disable_enable(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    args = ctx.args
    if len(args) < 2:
        await update.message.reply_text("Usage: /account disable <vault_id|telegram_id|@username>")
        await update.message.reply_text("Usage: /account enable <vault_id|telegram_id|@username>")
        return
    action = args[0].lower()
    identifier = " ".join(args[1:])
    if action not in ("disable","enable"):
        await update.message.reply_text("First argument must be 'disable' or 'enable'.")
        return
    user = None
    if identifier.startswith("@"):
        user = get_user_by_username(identifier)
    ...
    with get_db() as c:
        c.execute("UPDATE users SET disabled=? WHERE vault_id=?", (new_disabled, user["vault_id"]))
        c.commit()
    await update.message.reply_text(f"✅ Account {action}d.")
...
async def broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not users:
        ...
        return
    # If replying to a message, broadcast that message content
    if update.message.reply_to_message:
        ...
        for u in users:
            try:
                if msg.text:
                    await ctx.bot.send_message(chat_id=u["telegram_id"], text=msg.text, parse_mode="MarkdownV2" if msg.entities else None)
                elif msg.photo:
                    await ctx.bot.send_photo(chat_id=u["telegram_id"], photo=msg.photo[-1].file_id, caption=msg.caption)
                elif msg.video:
                    await ctx.bot.send_video(chat_id=u["telegram_id"], video=msg.video.file_id, caption=msg.caption)
                elif msg.document:
                    await ctx.bot.send_document(chat_id=u["telegram_id"], document=msg.document.file_id, caption=msg.caption)
                elif msg.audio:
                    await ctx.bot.send_audio(chat_id=u["telegram_id"], audio=msg.audio.file_id, caption=msg.caption)
                else:
                    await ctx.bot.send_message(chat_id=u["telegram_id"], text="[Broadcast]")
                success += 1
        ...
        return
    # Otherwise, use text from command
    if not ctx.args:
    ...
    for u in users:
        try:
            await ctx.bot.send_message(chat_id=u["telegram_id"], text=text, parse_mode="MarkdownV2")
            success += 1
            await asyncio.sleep(0.05)
        except Exception as e:
            logger.warning(f"Broadcast to {u['telegram_id']} failed: {e}")
...
async def userall_export(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await admin_only(update, ctx): return
    with get_db() as c:
        users = c.execute("SELECT telegram_id, tg_name, vault_id FROM users ORDER BY telegram_id").fetchall()
    lines = []
    for idx, u in enumerate(users, 1):
        totp_count = c.execute("SELECT COUNT(*) as n FROM totp_accounts WHERE vault_id=?", (u["vault_id"],)).fetchone()["n"]
        lines.append(f"{idx} — {u['telegram_id']} ~ {u['tg_name'] or 'Unknown'} ~ {u['vault_id']} ~ {totp_count}")
    content = "\n".join(lines)
    bio = BytesIO(content.encode())
    bio.name = "user_export.txt"
    await update.message.reply_document(document=bio, filename="user_export.txt", caption="📊 User list export")
    await update.message.reply_text("✅ Export complete.")

# ── /start (with share token) ───────────────────────────────
...
async def signup_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_signup_enabled():
        await q.edit_message_text("❌ Sign-up is currently disabled by the administrator.")
        return AUTH_MENU
    uid = update.effective_user.id
...
async def signup_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        parse_mode="MarkdownV2", reply_markup=kb_main())
    async def _delete_sk_msg(): await asyncio.sleep(300); await sk_msg.delete()
...
async def login_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_login_enabled():
        await q.edit_message_text("❌ Login is currently disabled by the administrator.")
        return AUTH_MENU
    await q.edit_message_text(
        "🔑 *Login*\n\nChoose how to login:",
        parse_mode="MarkdownV2",
...
async def login_pw(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        ...
        return AUTH_MENU
    if is_account_disabled(vid):
        await update.message.reply_text("⛔ This account has been disabled by the administrator.")
        return AUTH_MENU
    if is_login_frozen(vid):
        remaining, _ = get_login_freeze_remaining(vid)
        h, m = remaining // 3600, (remaining % 3600) // 60
        await update.message.reply_text(f"🔒 *Account temporarily locked*\\. Try again in {h}h {m}m.",
                                        parse_mode="MarkdownV2", reply_markup=kb_auth())
        return AUTH_MENU
    if not hmac.compare_digest(hash_pw(pw, bytes(u["pw_salt"])), bytes(u["password_hash"])):
        ...
        if frozen:
            remaining, _ = get_login_freeze_remaining(vid)
            h, m = remaining // 3600, (remaining % 3600) // 60
            await update.message.reply_text(f"🔒 *Account locked for {h}h {m}m* due to {MAX_LOGIN_ATTEMPTS} failed attempts.",
                                            parse_mode="MarkdownV2", reply_markup=kb_auth())
            return AUTH_MENU
        else:
            with get_db() as c:
            ...
            await update.message.reply_text(f"❌ Wrong password\\. {left} attempt(s) remaining before lockout.",
                                            parse_mode="MarkdownV2", reply_markup=kb_cancel())
        return LOGIN_PASSWORD
...
async def reset_id_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    vid = u["vault_id"]
    if is_account_disabled(vid):
        await update.message.reply_text("⛔ This account is disabled. Contact administrator.")
        return AUTH_MENU
...
async def _do_save_totp(update, vault, data, pw):
    ...
    with get_db() as c:
        c.execute("INSERT INTO totp_accounts (vault_id, name, issuer, secret_enc, salt, iv, "
...
async def _render_list_page(q_or_msg, vault: str, pw: str, page: int, is_edit: bool = True):
    ...
    chunk = rows[page * ITEMS_PER_PAGE : (page+1)*ITEMS_PER_PAGE]
    entries = []
    for i, row in enumerate(chunk, start=page*ITEMS_PER_PAGE + 1):
        try:
...
async def search_totp_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        ...
        "Use `#tag` to search in name and note fields.\n"
        "Example: `#github`\n\n"
        "_Your message will auto-delete after processing._",
        parse_mode="MarkdownV2", reply_markup=kb_cancel())
...
async def search_totp_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not vault or not pw:
        await update.message.reply_text("Session expired\\. /start", parse_mode="MarkdownV2", reply_markup=kb_auth())
        return AUTH_MENU
    # Search in name and note (case-insensitive)
    with get_db() as c:
        rows = c.execute(
            "SELECT id, name, issuer, secret_enc, salt, iv, note, account_type, hotp_counter "
    ...
    if not rows:
        await update.message.reply_text(f"🔍 No results for `{em(query)}`", parse_mode="MarkdownV2", reply_markup=kb_main())
        return TOTP_MENU
    # Display results (paginated)
    ctx.user_data["search_results"] = rows
    ctx.user_data["search_page"] = 0
    await _render_search_page(update.message, ctx, vault, pw, 0)
    return TOTP_MENU

async def _render_search_page(msg, ctx, vault, pw, page: int):
    rows = ctx.user_data.get("search_results", [])
    if not rows:
        await msg.reply_text("No results.", reply_markup=kb_main())
        return
    total = len(rows)
    total_pages = max(1, (total + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE)
    page = max(0, min(page, total_pages-1))
    chunk = rows[page*ITEMS_PER_PAGE : (page+1)*ITEMS_PER_PAGE]
    lines = [f"🔍 *Search Results* ({page+1}/{total_pages})\n"]
    for i, row in enumerate(chunk, start=page*ITEMS_PER_PAGE+1):
        try:
            secret = decrypt(row["secret_enc"], row["salt"], row["iv"], pw, vault)
            acc_type = row["account_type"] or "totp"
            ...
            if row["issuer"]:
                name_line += f" \\| _{em(row['issuer'])}_"
            lines.append(f"{name_line}\nCurrent Code: `{code_fmt}` {time_info}")
            if row["note"]:
                lines.append(f"_Note:_ {em(row['note'])}")
            lines.append("")
        except Exception as e:
            lines.append(f"*{i}\\. {em(row['name'])}*\n_\\[Decrypt error\\]_")
    kb = build_search_results_kb(list(range(total_pages)), page, total_pages)
    await msg.edit_text("\n".join(lines), parse_mode="MarkdownV2", reply_markup=kb)

async def search_page_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if q.data == "search_noop":
        return TOTP_MENU
    page = int(q.data.split("_")[2])
    ctx.user_data["search_page"] = page
    uid = update.effective_user.id
    vault = get_session(uid)
    pw = ctx.user_data.get("password")
    if not vault or not pw:
        await q.edit_message_text("Session expired\\. /start", parse_mode="MarkdownV2", reply_markup=kb_auth())
        return AUTH_MENU
    await _render_search_page(q, ctx, vault, pw, page)
    return TOTP_MENU
...
async def edit_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "rename":
        ...
        await q.edit_message_text(f"📝 *Add Note to {em(name)}*\n\n{note_info}Enter a note (max {NOTE_MAX_LEN} characters).\nSend a single space or `.` to clear the note:",
                                  parse_mode="MarkdownV2", reply_markup=kb_cancel())
        return NOTE_INPUT
    else:
        name = ctx.user_data.get("edit_name", "")
...
async def note_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        c.execute("UPDATE totp_accounts SET note=? WHERE id=? AND vault_id=?", (note, acc_id, vault)); c.commit()
    if note:
        await update.message.reply_text(f"✅ Note saved: _{em(note)}_", parse_mode="MarkdownV2", reply_markup=kb_main())
    else:
        await update.message.reply_text("✅ Note cleared.", parse_mode="MarkdownV2", reply_markup=kb_main())
    return TOTP_MENU
...
async def export_pw2_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id; vault = get_session(uid); pw = ctx.user_data.get("password", "")
    with get_db() as c:
        rows = c.execute("SELECT name, issuer, secret_enc, salt, iv, note, account_type, hotp_counter FROM totp_accounts WHERE vault_id=?", (vault,)).fetchall()
    if not rows:
    ...
    for row in rows:
        try:
            ...
            entries.append({"name": row["name"], "issuer": row["issuer"] or "", "secret": secret,
                            "note": row["note"] or "", "account_type": row["account_type"] or "totp",
                            "hotp_counter": row["hotp_counter"] or 0})
        except Exception as e:
            logger.error(f"Export decrypt: {e}")
    plain = json.dumps({"version": 4, "vault_id": vault, "accounts": entries}, ensure_ascii=False).encode()
        ...
        for acc in accounts:
            ...
            try:
                ...
                if sk:
                    ...
                    sk_ct = sk_s = sk_iv = None
                c.execute("INSERT INTO totp_accounts (vault_id, name, issuer, secret_enc, salt, iv, sk_enc, sk_salt, sk_iv, note, account_type, hotp_counter) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                          (vault, name, acc.get("issuer", ""), ct, s, iv, sk_ct, sk_s, sk_iv,
                           acc.get("note", ""), acc.get("account_type", "totp"), acc.get("hotp_counter", 0)))
                imported += 1
            except Exception as e:
...
async def cancel_to_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for k in [
        ...
        "share_selected","share_rows","search_results","search_page","list_page"
    ]:
        ctx.user_data.pop(k, None)
    uid = update.effective_user.id; vault = get_session(uid)
    if vault:
...
# ── MAINTENANCE CHECK ──────────────────────────────────────
async def check_maintenance(update: Update, ctx: ContextTypes.DEFAULT_TYPE, next_handler):
    if is_maintenance() and update.message and update.message.chat_id != GROUP_ID:
        await update.message.reply_text(
            "🛠 *BlockVeil Authenticator is currently in maintenance mode.*\n\n"
...
def main():
    ...
    if not GROUP_ID:
        logger.warning("GROUP_ID not set. Admin commands will not work.")
    init_db()
    ...
    # Admin commands (only in admin group)
    admin_filter = filters.Chat(chat_id=GROUP_ID) if GROUP_ID else filters.ALL
    app.add_handler(CommandHandler("maintenance", maintenance_on, filters=admin_filter & filters.Regex(r"^on$"), block=False))
    app.add_handler(CommandHandler("maintenance", maintenance_off, filters=admin_filter & filters.Regex(r"^off$"), block=False))
    app.add_handler(CommandHandler("export", export_all_data, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("import", import_all_data, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("user", user_info, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("sign-up", signup_toggle, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("login", login_toggle, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("account", account_disable_enable, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("broadcast", broadcast, filters=admin_filter, block=False))
    app.add_handler(CommandHandler("userall", userall_export, filters=admin_filter, block=False))
    # Admin import file/password handlers
    app.add_handler(MessageHandler(filters.Document.ALL & admin_filter, admin_import_file), group=1)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & admin_filter, admin_import_pw), group=1)
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            TOTP_MENU: [
                CallbackQueryHandler(add_totp_start, pattern="^add_totp$"), CallbackQueryHandler(list_totp, pattern="^list_totp$"),
                CallbackQueryHandler(list_page_cb, pattern=r"^list_page_\d+$"), CallbackQueryHandler(list_page_cb, pattern="^list_noop$"),
                CallbackQueryHandler(edit_totp_start, pattern="^edit_totp$"), CallbackQueryHandler(show_profile, pattern="^profile$"),
                CallbackQueryHandler(settings_menu, pattern="^settings$"), CallbackQueryHandler(change_pw_start, pattern="^change_pw$"),
                ...
                CallbackQueryHandler(export_vault_start, pattern="^export_vault$"), CallbackQueryHandler(import_vault_start, pattern="^import_vault$"),
                CallbackQueryHandler(delete_account_start, pattern="^delete_account$"), CallbackQueryHandler(logout, pattern="^logout$"),
                CallbackQueryHandler(main_menu_cb, pattern="^main_menu$"), CallbackQueryHandler(change_tz_start, pattern="^change_tz$"),
                ...
                CallbackQueryHandler(backup_reminder_menu, pattern="^backup_reminder$"), CallbackQueryHandler(backup_rem_toggle, pattern="^backup_rem_toggle$"),
                CallbackQueryHandler(backup_rem_freq, pattern="^backup_rem_freq$"),
                CallbackQueryHandler(search_totp_start, pattern="^search_totp$"), CallbackQueryHandler(search_page_cb, pattern=r"^search_page_\d+$"), CallbackQueryHandler(search_page_cb, pattern="^search_noop$"),
                CallbackQueryHandler(share_codes_open, pattern="^share_codes_open$"), CallbackQueryHandler(share_toggle, pattern=r"^share_toggle_\d+$"),
                CallbackQueryHandler(share_generate, pattern="^share_generate$"), CallbackQueryHandler(share_cancel, pattern="^share_cancel$"),
            ],
            ...
            NOTE_INPUT: [MessageHandler(private & filters.TEXT & ~filters.COMMAND, note_input), CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$")],
            ...
            SEARCH_WAITING: [MessageHandler(private & filters.TEXT & ~filters.COMMAND, search_totp_input), CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$")],
        },
    ...
    # Daily backup reminder job
    job_queue = app.job_queue
    if job_queue:
        job_queue.run_repeating(lambda ctx2: asyncio.create_task(send_backup_reminders(app)), interval=86400, first=60, name="backup_reminder_job")
...
async def send_backup_reminders(app):
    ...
    month_s = 30 * 24 * 3600
    with get_db() as c:
        rows = c.execute("SELECT telegram_id, frequency, last_sent FROM backup_reminders WHERE enabled=1").fetchall()
    for row in rows:
        interval = week_s if row["frequency"] == "weekly" else month_s
        ...
        try:
            ...
            with get_db() as c:
                c.execute("UPDATE backup_reminders SET last_sent=? WHERE telegram_id=?", (now, row["telegram_id"]))
                c.commit()
        except Exception as e:
            logger.warning(f"Backup reminder failed for {row['telegram_id']}: {e}")
